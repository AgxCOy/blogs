...
虽然经过与 Zero Fanker 等人的讨论后决定做些<del>修缮</del>重写，但难免仍有需要改进之处。
:::
...
::: info 扩展变量系统
船新的变量系统大大扩充了局部变量池，几乎可以认为是“无限量”使用。<del>真的有人会堆料到要用 2^31^ - 1 个以上的变量吗？</del>
...
print(x + y)
```
可以很清楚地看出，这段程序先读入`x`，后读入`y`，最后输出它们两相加<del>（实际上是拼接）</del>的结果。**自上而下、逐行执行**，这就是程序代码的顺序结构。
...
![英文原版直接称作 Type<br>早期汉化也直接译作“类型”](.img/ra2_trigger_logic-trigger_editor.webp)
...
分析这个运行表现不难发现，它是类似下图图二的流程：首先它走到`while`处执行判断，由于条件恒满足，往下执行`print`；然后循环并没有结束，它重新回到`while`重复执行前面说的流程，将坏掉的乐土打字机事业推进下去<del>爱莉希雅死辣</del>。
